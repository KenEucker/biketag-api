...
describe(`Built Module`, () => {
  test(`Resolves proper node build`, () => {
    expect(require.resolve('biketag')).toBe(path.join(rootPath, 'biketag.node.js'))
  })

  test(`Module exports BikeTagClient as default and named export`, () => {
    const { default: defaultExport, BikeTagClient: namedExport } = require('biketag/biketag.node.js');
